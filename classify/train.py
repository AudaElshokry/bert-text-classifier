...
from classify.utils import seed_everything, read_csv_required, build_label_maps, apply_label_map, save_label_map
...
def main():
    ...
    # Respect user-requested GPU IDs (if available)
    ...
    # Label maps built from TRAIN only; apply consistently to val/test
    ...
    # Model
    model = build_model(args.bert_model, num_labels=num_labels, id2label=id2label, label2id=label2id)
    ...
    # Train (with early stopping + best checkpoint)
    trainer.train(output_dir=args.output_path)

    # Evaluate on test, save predictions, report & metrics
